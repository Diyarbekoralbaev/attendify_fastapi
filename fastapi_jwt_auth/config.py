...
class LoadConfig(BaseModel):
    ...
    @field_validator('authjwt_access_token_expires', mode='before')
    def validate_access_token_expires(cls, v):
        if v is True:
            raise ValueError("The 'authjwt_access_token_expires' only accepts value False (bool)")
        return v

    @field_validator('authjwt_refresh_token_expires', mode='before')
    def validate_refresh_token_expires(cls, v):
        if v is True:
            raise ValueError("The 'authjwt_refresh_token_expires' only accepts value False (bool)")
        return v
    ...
    @field_validator('authjwt_cookie_samesite', mode='before')
    ...
    def validate_csrf_methods(cls, v):
        if v.upper() not in {"GET", "HEAD", "POST", "PUT", "DELETE", "PATCH"}:
            raise ValueError("The 'authjwt_csrf_methods' must be valid HTTP request methods")
        return v.upper()

    class Config:
        str_min_length = 1  # Updated from min_anystr_length
        str_strip_whitespace = True  # Updated from anystr_strip_whitespace
        from_attributes = True  # If needed for ORM compatibility
        # json_schema_extra = {}  # Uncomment and set if you need schema extra information
